...
| Webserv           | A project to build an HTTP server in C++98, supporting multiple clients, HTTP methods (GET, POST, DELETE), and handling static websites and CGI. | C++, HTTP, Networking, Sockets, CGI, Git collaboration | group      | in progress         |
| Inception         | A system administration project that involves setting up a multi-service infrastructure using Docker containers. Includes services like NGINX, WordPress, and MariaDB, all running within a virtualized environment. | Docker, Docker Compose, NGINX, MariaDB, WordPress | solo       | in progress         |
| ft_transcendence  | A web-based multiplayer Pong game project that involves creating a real-time gaming experience with user authentication, security features, and additional modules such as AI, matchmaking, and game customization. | JavaScript, Ruby, WebSockets, Docker, HTML/CSS, Git collaboration | group      | TBD (not started yet) |
